...
# /close message:
@tree.command(name="dadah", description="Tutup chat dengan Ruri")
...
async def dadah_command(int: discord.Interaction, message: str, thread: discord.Thread):
    if not isinstance(int.channel, discord.Thread):
        return
    
    user = int.user
    logger.info(f"Dadah command by {user} {message[:20]}")

    await thread.edit(name=INACTIVATE_THREAD_PREFIX)
    await thread.send(
        embed=discord.Embed(
            description="**Thread closed** - Dadah {user}, closing...",
